...
angular.module('mustache.controllers', [])
    .controller('MustacheCtrl', ['$scope', 'mustacheService',
        function($scope, mustacheService) {
            ...
            $scope.syntaxNames;
            $scope.tabSize = 4;
		  	$scope.indentRange = ['1','2','3','4','5','6','7','8'];
            ...
            $scope.showTab = function() {
                var iCurrentEditor = mustacheService.getCurrentEditor();
                if (iCurrentEditor) {
                    editors[ed - 1].scrollTo({
                        x: $scope.workingFiles[iCurrentEditor].scrollView
                    })
            ...
            $scope.fnNewFile = function(filename, fullPath, fileExtension) {
				...
				try {
				  ...
				  console.log('Unable to initialize CodeMirror on first try, attempting again');
				  $scope.fnCreateEditorInstance("editor-" + ed);
				  $('.CodeMirror').css({
					  "font-size": $scope.fontsize + "px"
				  })
            ...
            $scope.fnCloseFile = function(file) {
                ...
                $('#tabs').find('.codeeditor' + iCurrentEditor).remove();
			  	$('.tab-content').find('.codeeditor' + iCurrentEditor).remove()
            ...
            $scope.fnSaveFile = function() {
                $scope.editorStatus = mustacheService.saveFile($scope.workingFiles);
                $scope.$apply($scope.editorStatus);
            }
            $scope.fnSaveFileAs = function() {
                $scope.editorStatus = mustacheService.saveAs();
                $scope.$apply($scope.editorStatus);
            }
            ...
            $scope.fnToggleLineNumbers = function() {
                if ($scope.toggleLineNumbers) {
                    ...
                    if (editors.length === 0) {
                        ...
                        editors[iCurrentEditor].setOption({
            ...
            $scope.fnBeautifyCode = function() {
                mustacheService.beautifyCode();
            }
            ...
            key('ctrl+shift+b', function() {
                $scope.editorStatus = mustacheService.beautifyCode();
                return false
            });
